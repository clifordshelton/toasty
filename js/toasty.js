   ...
   var defaults = {
    ...
    function showToast(params) {
        try{
            ...
            toastMessage.find(".closeToast").on("click", function(){
                hideToast(item);
            });
    ...
    function hideToast(item) {
        ...
        if (activeMsgs[item.msgID]) {
            ...
            if (!activeItem.hiding && activeItem.toast != null) {
                ...
                animateToast(activeItem, false);
            }
        }
    }

    function animateToast(item, show) {
        ...
        try{
            if (show) {
                ...
                if (item.beforeHide != null) {
                    try{ beforeResponse = item.beforeHide(item); }catch(ex){}
                }
            }
        } catch(ex) {}
            ...
            item.animHandle = item.toast.animate(animParam, duration, easing, function () {
                ...
                if (show) {
                    ...
                    if (item.afterHide != null) {
                        try{ item.afterHide(item); }catch(ex){}
                    }
                    item.toast.remove();
                    item.toast = null;
                    activeMsgOrder.splice(activeMsgOrder.indexOf(item.msgID), 1);
                    if (activeMsgOrder.length == 0) {
                        $(document).unbind("keydown", handleEscapeKey);
                    }
                    delete activeMsgs[item.msgID];
                    if (item.afterDestroy != null) {
                        try{ item.afterDestroy(item); }catch(ex){}
                    }
                }
            });
        }
    ...
    function handleEscapeKey(e){
        ...
        if (e.keyCode == 27) {
            for (i = 0, maxI = activeMsgOrder.length; i < maxI; i++) {
                if (activeMsgs[activeMsgOrder[i]].closeable) {
                    hideToast(activeMsgs[activeMsgOrder[i]]);
                    break;
                }
            }
        }
